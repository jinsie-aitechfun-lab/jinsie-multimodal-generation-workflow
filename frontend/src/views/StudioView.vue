...
// Pulls a human-readable title (the story title generated by the LLM)
// out of the current workflow response. Returns undefined if the
// response has no story output yet — caller should pass undefined
// rather than guess, so the history card falls back to "视频 N"
// rather than show stale text from a previous run.
function deriveCurrentVideoMeta(): RecentVideoMeta | undefined {
  const story = currentWorkflowResponse.value?.outputs?.story as
  ...
  finalVideoAudioEnabled.value = (data.outputs as any)?.final_video?.audio_enabled ?? true
  {
    ...
    if (extractedVideoUrl && deletedVideoUrlsSet.value.has(extractedVideoUrl)) {
      ...
      if (finalVideoUrl.value) {
        pushRecentFinalVideoUrl(finalVideoUrl.value, deriveCurrentVideoMeta())
