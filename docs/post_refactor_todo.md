...
## BUG-009: 视觉验证器把小蝌蚪识别成青蛙，导致候选图被错误淘汰或评分偏低

**发现日期**：2026-05-26（用户反馈）

**触发条件**：

主题包含小蝌蚪、幼体动物或其他与成体外观差异大的角色，例如：
...
视觉验证器（`image_visual_verifier`）调用视觉大模型对候选图评分时，把图中的小蝌蚪识别为青蛙，导致：

- `required_characters` 检查中，角色"小蝌蚪"未被识别 → `missing_required_characters` 硬失败 → 候选图分数低于 `MIN_PASS_SCORE`，触发不必要的 `should_retry`
- 或者反过来：图中确实画了青蛙，视觉验证器却误认为小蝌蚪已出现

**根因**：

视觉验证器的 prompt 只传入了角色名（"小蝌蚪"），没有补充外观描述说明这是幼体动物，与成体青蛙外观差异极大。视觉大模型根据自身知识把"黑色小圆体+尾巴"归类为青蛙而非蝌蚪。

**建议修复**：

- 在传给视觉验证器的角色描述中，附加 `visual_identity`（如"黑色圆形小体、细长尾巴、无四肢"），而不只是 `display_name`
- 在 `character_visual_profile_llm.py` 或 `character_visual_profile.py` 为幼体动物生成与成体不同的外观描述
- 视觉验证器的 prompt 中增加"如果图中出现的动物形态与名称描述一致（即使与成体不同），视为识别正确"的说明

**优先级**：P1。影响所有幼体/变形动物主题的候选图筛选准确性。
